...
export function setInnerWithIndentation (source: MagicString, el: { start: number, end: number, innerStart: number, innerEnd: number }, injection: string | any) {
  const style = detectStyle(source.original);
  const detectedSpace = detectSpace(source.original, el.start);
  let toInject: string;
...
export function getOrCreateTag (source: MagicString, els: ParsedTag[], detect: (el: ParsedTag) => boolean, injection: string | null): { source: MagicString, els: ParsedTag[], el: ParsedTag } {
  for (const el of els) {
    if (detect(el))
